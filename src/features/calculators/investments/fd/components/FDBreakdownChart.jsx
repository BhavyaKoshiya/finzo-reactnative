...
export const FDBreakdownChart = ({ result, style }) => {
  const { currentTheme } = useAppTheme();

  if (!result) return null;

  const { principal, interestEarned, maturityAmount } = result;
  const total = maturityAmount || (principal + interestEarned) || 1;

  const principalPct = Math.round((principal / total) * 100);
  const interestPct = Math.max(100 - principalPct, 0);
...
export default FDBreakdownChart;
